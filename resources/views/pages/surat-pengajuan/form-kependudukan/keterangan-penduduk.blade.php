<div id="form-keterangan-penduduk">
    @isset($suratEdit)
    @endisset
    <div class="intro-y mt-5 col-span-12 lg:col-span-12">
        ...
        <div class="intro-y box p-5">
            ...
            @endphp
            <form id="createFormKeteranganPenduduk" action="{{ route($url_form_action) }}" method="POST">
                @csrf
                ...
                <div class="card p-4 mb-4" style="background-color: #9fa8b3; color: black; border-radius: 10px">
                    ...
                    <div class="card-body">
                        <div class="grid grid-cols-2 gap-4 mt-3">
                            ...
                            <!-- Kelurahan -->
                            <div>
                                <x-base.form-label for="form-kel-keterangan-penduduk">Kelurahan/Desa <span
                                        class="text-red-500">*</span></x-base.form-label>
                                <x-base.form-select class="sm:mr-2" id="form-kel-keterangan-penduduk"
                            ...
                            <!-- Kecamatan -->
                            <div>
                                <x-base.form-label for="form-kec-keterangan-penduduk">Kecamatan <span
                                        class="text-red-500">*</span></x-base.form-label>
                                <x-base.form-select class="sm:mr-2" id="form-kec-keterangan-penduduk"
                ...
                <div class="mt-5 text-right">
                    ...
                    <x-base.button id="save-button-ket-penduduk" class="w-24 text-white" type="button"
                        variant="success">
                        Simpan
                    </x-base.button>
                </div>
...
<div id="confirmation-modal-ket-penduduk" aria-hidden="true" tabindex="-1"
    ...
    <div
        ...
        <div class="px-5 pb-8 text-center flex justify-center space-x-2">
            <x-base.button id="cancel-button-ket-penduduk" class="w-full px-4 py-3" variant="soft-dark"
                type="button">
                Periksa Kembali
            </x-base.button>
            <x-base.button id="confirm-button-ket-penduduk" class="w-full px-4 py-3 text-white" variant="success"
                type="button">
                Simpan
            </x-base.button>
        </div>
...
<script>
    ...
    const formSelectKecKeteranganPenduduk = document.getElementById('form-kec-keterangan-penduduk')
    const formSelectKelKeteranganPenduduk = document.getElementById('form-kel-keterangan-penduduk')

    formSelectKecKeteranganPenduduk.addEventListener("change", function() {
        var selectedKec = this.value; // Get the value of the selected kecamatan
        var kelOptions = document.querySelectorAll(
            "#form-kel-keterangan-penduduk option.data-kel"); // Select all options in kelurahan
        ...
        formSelectKelKeteranganPenduduk.value = ''
        formSelectKelKeteranganPenduduk.dispatchEvent(new Event('change'));
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (isadmin != 'true') {
            console.log('keganti')
            formSelectKecKeteranganPenduduk.value = `{{ $user->id_kec }}`;
            formSelectKecKeteranganPenduduk.dispatchEvent(new Event('change'));
            formSelectKelKeteranganPenduduk.value = `{{ $user->id_kel }}`;
            formSelectKelKeteranganPenduduk.dispatchEvent(new Event('change'));
        }
    })

    document.getElementById('save-button-ket-penduduk').addEventListener('click', function(event) {
        event.preventDefault(); // Prevent the form from submitting

        const form = document.getElementById('createFormKeteranganPenduduk');

        if (form.checkValidity()) {
            // Form is valid, show the modal
            const modal = document.getElementById('confirmation-modal-ket-penduduk');
    ...
    document.getElementById('cancel-button-ket-penduduk').addEventListener('click', function() {
        const modal = document.getElementById('confirmation-modal-ket-penduduk');
    ...
    document.getElementById('confirm-button-ket-penduduk').addEventListener('click', function() {
        document.getElementById('createFormKeteranganPenduduk').submit(); // Submit the form
    });
</script>
