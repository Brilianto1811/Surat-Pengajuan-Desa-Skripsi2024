<div id="form-jalan">
    ...
    <div class="intro-y mt-5 col-span-12 lg:col-span-6">
        ...
        <div class="intro-y box p-5">
            ...
            @endphp
            <form id="createFormJalan" action="{{ route($url_form_action) }}" method="POST">
                @csrf
                ...
                <div class="card p-4 mb-4" style="background-color: #9fa8b3; color: black; border-radius: 10px">
                    ...
                    <div class="card-body">
                        <div class="grid grid-cols-2 gap-4 mt-3">
                            ...
                            <!-- Kelurahan -->
                            <div>
                                <x-base.form-label for="form-kel-jalan">Kelurahan/Desa <span
                                        class="text-red-500">*</span></x-base.form-label>
                                <x-base.form-select class="sm:mr-2" id="form-kel-jalan" aria-label="Surat"
                            ...
                            <!-- Kecamatan -->
                            <div>
                                <x-base.form-label for="form-kec-jalan">Kecamatan <span
                                        class="text-red-500">*</span></x-base.form-label>
                                <x-base.form-select class="sm:mr-2" id="form-kec-jalan" aria-label="Surat"
                ...
                <div class="mt-5 text-right">
                    ...
                    <x-base.button id="save-button-jalan" class="w-24 text-white" type="button" variant="success">
                        Simpan
                    </x-base.button>
                </div>
            </form>
...
<div id="confirmation-modal-jalan" aria-hidden="true" tabindex="-1"
    ...
    <div
        ...
        <div class="px-5 pb-8 text-center flex justify-center space-x-2">
            <x-base.button id="cancel-button-jalan" class="w-full px-4 py-3" variant="soft-dark" type="button">
                Periksa Kembali
            </x-base.button>
            <x-base.button id="confirm-button-jalan" class="w-full px-4 py-3 text-white" variant="success"
                type="button">
                Simpan
            </x-base.button>
        </div>
...
<script>
    const formSelectKecJalan = document.getElementById('form-kec-jalan')
    const formSelectKelJalan = document.getElementById('form-kel-jalan')

    formSelectKecJalan.addEventListener("change", function() {
        var selectedKec = this.value; // Get the value of the selected kecamatan
        var kelOptions = document.querySelectorAll(
            "#form-kel-jalan option.data-kel"); // Select all options in kelurahan
        ...
        formSelectKelJalan.value = ''
        formSelectKelJalan.dispatchEvent(new Event('change'));
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (isadmin != 'true') {
            console.log('keganti')
            formSelectKecJalan.value = `{{ $user->id_kec }}`;
            formSelectKecJalan.dispatchEvent(new Event('change'));
            formSelectKelJalan.value = `{{ $user->id_kel }}`;
            formSelectKelJalan.dispatchEvent(new Event('change'));
        }
    })

    document.getElementById('save-button-jalan').addEventListener('click', function(event) {
        event.preventDefault(); // Prevent the form from submitting

        const form = document.getElementById('createFormJalan');

        if (form.checkValidity()) {
            // Form is valid, show the modal
            const modal = document.getElementById('confirmation-modal-jalan');
    ...
    document.getElementById('cancel-button-jalan').addEventListener('click', function() {
        const modal = document.getElementById('confirmation-modal-jalan');
    ...
    document.getElementById('confirm-button-jalan').addEventListener('click', function() {
        document.getElementById('createFormJalan').submit(); // Submit the form
    });
</script>
